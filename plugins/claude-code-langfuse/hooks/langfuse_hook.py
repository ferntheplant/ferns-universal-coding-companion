...
Runs on Stop (and optionally PreToolUse/PostToolUse). Reads the session
transcript incrementally and emits one hierarchical Langfuse trace per turn:
  trace (turn) -> generation (each assistant message) -> tool spans

Shared metadata schema (consistent across pi, codex, opencode harnesses):
  harness, sessionId, cwd, repo, repoRemote, gitBranch, gitCommit, model
"""
...
def get_git_context(cwd: Optional[str] = None) -> Dict[str, str]:
    """Capture git metadata from cwd for the shared base schema."""
...
def build_trace_tags(git_ctx: Dict[str, str], model: str) -> List[str]:
    tags = ["harness:claude-code"]
    if model and model != "claude":
        tags.append(f"model:{model}")
    if git_ctx.get("repo"):
        tags.append(f"repo:{git_ctx['repo']}")
    return tags
...
def emit_turn(
    ...
    # Determine primary model from last assistant message
    primary_model = get_model(last_assistant)

    # Count tools and errors for scoring
    total_tool_uses = sum(len(iter_tool_uses(get_content(am))) for am in turn.assistant_msgs)
    total_tool_results = len(turn.tool_results_by_id)
    ...
    trace_tags = build_trace_tags(git_ctx, primary_model)

    with propagate_attributes(
        session_id=session_id,
        trace_name=f"claude-code:turn",
        tags=trace_tags,
    ):
    ...
    tool_success_rate = (total_tool_results / total_tool_uses) if total_tool_uses > 0 else 1.0
    try:
        trace_id = getattr(trace_span, "trace_id", None)
        if trace_id:
            langfuse.score(trace_id=trace_id, name="tool_call_count", value=float(total_tool_uses), data_type="NUMERIC")
            langfuse.score(trace_id=trace_id, name="turn_count", value=1.0, data_type="NUMERIC")
            langfuse.score(trace_id=trace_id, name="tool_success_rate", value=tool_success_rate, data_type="NUMERIC")
    except Exception as e:
        debug(f"score emit failed: {e}")
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            for t in turns:
                ...
                try:
                    emit_turn(langfuse, session_id, turn_num, t, transcript_path, cwd, git_ctx)
                except Exception as e:
        ...
        dur = time.time() - start
        info(f"Processed {emitted} turns in {dur:.2f}s (session={session_id}, repo={git_ctx.get('repo', 'unknown')})")
        return 0
