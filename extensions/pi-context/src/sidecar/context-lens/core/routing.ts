...
export function resolveTargetUrl(
  ...
  if (!targetUrl) {
    ...
    // x-target-url is an absolute URL. If it already ends with the request
    // path (i.e. it's a complete endpoint URL), use it as-is. Otherwise
    // treat it as a base URL and append the path (e.g. opencode.ai/zen → /v1/messages).
    const parsedTarget = new URL(targetUrl);
    if (!parsedTarget.pathname.endsWith(parsedUrl.pathname)) {
      targetUrl = targetUrl.replace(/\/$/, "") + parsedUrl.pathname + search;
    }
  }
