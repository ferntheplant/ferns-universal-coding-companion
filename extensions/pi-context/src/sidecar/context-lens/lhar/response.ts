...
export function parseResponseUsage(responseData: any): ParsedResponseUsage {
  ...
  if (responseData.usage) {
    ...
    if (u.prompt_tokens_details?.cached_tokens) {
      ...
      // OpenAI's prompt_tokens includes cached tokens; subtract to normalize so
      // that inputTokens always represents non-cached input (like Anthropic/Gemini),
      // keeping the store.ts invariant: total = inputTokens + cacheReadTokens + cacheWriteTokens
      if (u.prompt_tokens) {
