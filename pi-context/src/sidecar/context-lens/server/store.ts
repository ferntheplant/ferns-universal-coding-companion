...
export class Store {
  ...
  storeRequest(
    ...
    if (sessionId) {
      ...
      // Pi resends full message history each turn and has no built-in session IDs.
      // We split sessions two ways:
      //   1. TTL: inactivity longer than PI_SESSION_TTL_MS starts a new session.
      //   2. Context shrinkage: if messagesTokens drops to <50% of the previous
      //      request's count, the user ran /new and the context was reset.
      const now = Date.now();
      ...
      const contextReset =
        ...
        currentMessagesTokens < tracked.lastMessagesTokens * 0.5;
      ...
      if (!timedOut && !contextReset) {
        ...
        conversationId = createHash("sha256")
          ...
          .digest("hex")
          .slice(0, 16);
        this.piSessionTracker.set(fingerprint, {
          conversationId,
          lastSeen: now,
          lastMessagesTokens: currentMessagesTokens,
        });
      }
    } else if (fingerprint) {
